...
function requestErrorMessage(error: unknown): string {
  ...
  if (error instanceof ApiError && error.errorCode === "POS_SETTLEMENT_TOTAL_MISMATCH") {
    return "결제 금액과 선택한 미수금 합계가 다릅니다.";
  }
  if (error instanceof ApiError && error.errorCode === "POS_SETTLEMENT_USAGE_ALREADY_ALLOCATED") {
    return "선택한 거래는 이미 다른 정산 기록에 포함되었습니다. 목록을 새로고침해 주세요.";
  }
  if (error instanceof ApiError && error.status === 409) {
    return "정산 기록이 현재 상태와 맞지 않습니다. 목록을 새로고침해 확인해 주세요.";
  }
  return "정산 기록 결과를 확인하지 못했습니다. 네트워크를 확인한 뒤 같은 입력으로 다시 시도해 주세요.";
}
...
export function PosSettlementForm() {
  ...
  const loadReceivables = useCallback(async (manual = false) => {
    ...
    try {
      const nextReceivables = await getOutstandingReceivables();
      if (requestEpoch !== requestEpochRef.current) return;
      setReceivables(nextReceivables);
      replaceSelectedUsageIds(new Set());
      resetIdempotencyKey();
      setReceivableViewState(nextReceivables.length === 0 ? "empty" : "ready");
    } catch (error) {
      if (requestEpoch !== requestEpochRef.current) return;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedMealContractId || selectedReceivables.length === 0) {
      setFormError("정산 기록에 포함할 미수금 거래를 하나 이상 선택해 주세요.");
      return;
    }
    if (!posBusinessDate) {
    ...
    if (submittedTotalMinor !== derivedTotalMinor) {
      setFormError("결제 금액과 선택한 미수금 합계가 다릅니다.");
      return;
    }
  ...
  if (accessState === "forbidden") {
    return <StatePanel title="접근 권한이 없습니다" description="이 계정으로는 정산 기록을 조회하거나 기록할 수 없습니다." />;
  }
      ...
      <section className={posSettlementFormStyles.container} aria-labelledby="pos-settlement-title">
        <header className={posSettlementFormStyles.header}>
          <div>
            ...
            <h1 className={posSettlementFormStyles.title} id="pos-settlement-title">정산 기록</h1>
            <p className={posSettlementFormStyles.description}>
              외부 결제 결과를 저장합니다. 이 기록은 선불 잔액을 바꾸지 않습니다.
            </p>
          </div>
          <button
        ...
        <section className={posSettlementFormStyles.card} aria-labelledby="pos-settlement-record-title">
          {settlement ? (
            <div className={posSettlementFormStyles.success} role="status">
              <h2 className="sr-only" id="pos-settlement-record-title">정산 기록 저장 완료</h2>
              ...
              <ul className={posSettlementFormStyles.allocationList} aria-label="기록된 미수금 배분">
              ...
              <button className={posSettlementFormStyles.newSettlement} onClick={startAnotherSettlement} type="button">다른 정산 기록</button>
            </div>
          ) : receivableViewState === "loading" ? (
            <div className={posSettlementFormStyles.state}>
              <h2 className={posSettlementFormStyles.stateTitle} id="pos-settlement-record-title">미수금을 불러오는 중</h2>
              <p className={posSettlementFormStyles.stateDescription}>잠시만 기다려 주세요.</p>
            </div>
          ) : receivableViewState === "error" ? (
            <div className={posSettlementFormStyles.state}>
              <h2 className={posSettlementFormStyles.stateTitle} id="pos-settlement-record-title">미수금을 불러오지 못했습니다</h2>
              <p className={posSettlementFormStyles.stateDescription}>네트워크 상태를 확인한 뒤 다시 시도해 주세요.</p>
              <button className={posSettlementFormStyles.stateAction} onClick={() => void loadReceivables()} type="button">미수금 다시 불러오기</button>
            </div>
          ) : receivableViewState === "empty" ? (
            <div className={posSettlementFormStyles.state}>
              <h2 className={posSettlementFormStyles.stateTitle} id="pos-settlement-record-title">정산할 미수금 거래가 없습니다</h2>
              <p className={posSettlementFormStyles.stateDescription}>확정된 미수금이 생기면 여기에서 선택해 정산 기록을 저장할 수 있습니다.</p>
            </div>
          ) : (
            <>
              <h2 className="sr-only" id="pos-settlement-record-title">정산 기록</h2>
              {selectedMealContractId ? (
                <p className={posSettlementFormStyles.selectionHint} role="status">선택한 계약과 같은 계약의 미수금만 함께 기록할 수 있습니다.</p>
              ) : null}
              ...
              <div className={posSettlementFormStyles.tableWrap}>
                <table className={posSettlementFormStyles.table}>
                  <thead className={posSettlementFormStyles.tableHeader}>
                    <tr>
                      <th scope="col">선택</th>
                      <th scope="col">확정 거래</th>
                      <th scope="col" className="text-right">미수금</th>
                    </tr>
                  </thead>
                  <tbody>
                    {receivables.map((receivable) => {
                      const disabled = isSubmitting || Boolean(selectedMealContractId && selectedMealContractId !== receivable.mealContractId);
                      ...
                      return (
                        ...
                        >
                          <td>
                            <input
                              aria-label={
                                (receivable.partnerDisplayName ?? "협력사 정보 없음")
                                + " "
                          ...
                          <td>
                            <p className={posSettlementFormStyles.primary}>{receivable.partnerDisplayName ?? "협력사 정보 없음"}</p>
                            <p className={posSettlementFormStyles.secondary}>{dateFormatter.format(new Date(receivable.confirmedAt))}</p>
                          </td>
              ...
              <form className={posSettlementFormStyles.form} onSubmit={(event) => void submit(event)}>
                <h2 className={posSettlementFormStyles.formTitle}>선택한 미수금을 정산 기록으로 저장</h2>
                <p className={posSettlementFormStyles.formDescription}>선택 합계와 결제 금액이 같을 때만 저장됩니다.</p>
                ...
                <button className={posSettlementFormStyles.submit} disabled={isSubmitting || selectedReceivables.length === 0} type="submit">
                  {isSubmitting ? "정산 기록 저장 중…" : "정산 기록 저장"}
                </button>
              </form>
            </>
          )}
        ...
        <section className={posSettlementFormStyles.historyCard} aria-labelledby="pos-settlement-history-title">
          <header className={posSettlementFormStyles.historyHeader}>
            <div>
              <h2 className={posSettlementFormStyles.historyTitle} id="pos-settlement-history-title">최근 저장한 정산 기록</h2>
            </div>
            {historyViewState === "ready" || historyViewState === "empty" ? (
              <button
                ...
                type="button"
              >
                {isHistoryRefreshing ? "정산 기록 불러오는 중…" : "정산 기록 다시 불러오기"}
              </button>
            ) : null}
          </header>
          ...
          {historyViewState === "loading" ? (
            <div className={posSettlementFormStyles.state}>
              <p className={posSettlementFormStyles.stateDescription}>저장된 정산 기록을 불러오는 중입니다.</p>
            </div>
          ) : historyViewState === "error" ? (
            <div className={posSettlementFormStyles.state}>
              <p className={posSettlementFormStyles.notice} role="alert">저장된 정산 기록을 불러오지 못했습니다.</p>
              <button
                className={posSettlementFormStyles.stateAction}
                disabled={isHistoryRefreshing}
                onClick={() => void loadHistory(true)}
            ...
            <div className={posSettlementFormStyles.state}>
              <p className={posSettlementFormStyles.stateDescription}>최근 저장한 정산 기록이 없습니다.</p>
            </div>
          ) : (
            <ol className={posSettlementFormStyles.historyList} aria-label="최근 저장한 정산 기록 목록">
                    ...
                    <article>
                      ...
                      {isExpanded ? (
                        <div className={posSettlementFormStyles.historyDetail} id={detailId}>
                          ...
                          <ul className={posSettlementFormStyles.historyAllocationList} aria-label={"결제일 " + record.posBusinessDate + " 미수금 배분"}>
